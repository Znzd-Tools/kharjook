...
export interface FetchProviderQuotesResult {
...
function normalizeApiSources(sources?: ApiQuoteSource[]): RequestQuoteSource[] | undefined {
...
export async function fetchProviderQuotes(input: {
  slugs: string[];
  sources?: ApiQuoteSource[];
}): Promise<FetchProviderQuotesResult> {
  const requestedSlugs = input.slugs.filter((value): value is string => typeof value === 'string');
  const requestSources = normalizeApiSources(input.sources);
  ...
  try {
    ...
    if (sources.length === 0) {
      return { quotes: [], failedProviders: [], unresolvedSlugs: [], unknownRequestedSlugs };
    }

    const abanTetherSources = sources.filter((source) => source.provider === 'abantether');
    ...
    return {
      ...
      failedProviders: [
        {
          provider: 'server',
          error: error instanceof Error ? error.message : formatUpstreamError(error),
        },
      ],
      unresolvedSlugs: [],
      unknownRequestedSlugs,
